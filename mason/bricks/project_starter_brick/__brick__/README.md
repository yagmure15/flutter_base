...
> geçirdiği için boşluk içeren dizinlerde (bu repo: `{{name.titleCase()}}/`) kırılır. Bu yüzden SDK
...
```dart
// packages/core/lib/{{name.snakeCase()}}_core.dart
...
Bu, `{{name.snakeCase()}}_core.module.dart` içinde `{{name.pascalCase()}}CorePackageModule` sınıfını üretir;
...
productFlavors {
    dev {
        ...
        applicationIdSuffix = ".dev"        // com.example.{{name.snakeCase()}}.dev
        versionNameSuffix = "-dev"
        resValue "string", "app_name", "{{name.titleCase()}} Dev"
    }
    staging { ... }
    prod {
        dimension = "environment"
        resValue "string", "app_name", "{{name.titleCase()}}"
    }
}
```
...
```bash
mason add -g project_starter_brick git https://github.com/yagmure15/{{name.snakeCase()}}.git \
...
- İçerikte üç değiştirme yapılır: `{{name.snakeCase()}}` → `{{name.snakeCase()}}`,
  `{{name.titleCase()}}` → `{{name.titleCase()}}`, `{{name.pascalCase()}}` → `{{name.pascalCase()}}`
...
2. Barrel dosyasına (`{{name.snakeCase()}}_core.dart`) export ekleyin.
